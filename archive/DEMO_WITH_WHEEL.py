#!/usr/bin/env python3
"""
G29 + PIPER ARM - WITH VISIBLE VIRTUAL STEERING WHEEL
Now you can SEE the virtual steering wheel moving!
"""
...
print("="*70)
print(" G29 + PIPER ARM - WITH VISIBLE STEERING WHEEL")
print("="*70)
...
# ============================================================================
# CREATE VIRTUAL STEERING WHEEL (VISIBLE!)
# ============================================================================
print("\n[2/4] Creating virtual steering wheel...")

stage = omni.usd.get_context().get_stage()

# Steering wheel base (column)
wheel_base_path = "/World/WheelBase"
wheel_base = UsdGeom.Cylinder.Define(stage, wheel_base_path)
wheel_base.GetRadiusAttr().Set(0.03)
wheel_base.GetHeightAttr().Set(0.3)
wheel_base.GetAxisAttr().Set("Z")
UsdGeom.Xformable(stage.GetPrimAtPath(wheel_base_path)).AddTranslateOp().Set(Gf.Vec3d(-0.5, 0, 0.15))

# Steering wheel (torus approximated as cylinder)
wheel_path = "/World/SteeringWheel"
wheel = UsdGeom.Cylinder.Define(stage, wheel_path)
wheel.GetRadiusAttr().Set(0.15)
wheel.GetHeightAttr().Set(0.03)
wheel.GetAxisAttr().Set("Z")

print("✓ Virtual steering wheel created")

# ============================================================================
# CREATE ARM VISUALIZATION
# ============================================================================
print("\n[3/4] Creating arm visualization...")
...
UsdGeom.Xformable(stage.GetPrimAtPath(base_path)).AddTranslateOp().Set(Gf.Vec3d(0.5, 0, 0.05))
...
def update_steering_wheel(angle_deg):
    """Update steering wheel rotation"""
    wheel_xform = UsdGeom.Xformable(stage.GetPrimAtPath(wheel_path))
    wheel_xform.ClearXformOpOrder()
    wheel_xform.AddTranslateOp().Set(Gf.Vec3d(-0.5, 0, 0.3))
    wheel_xform.AddRotateZOp().Set(angle_deg)

def update_arm_visualization(q1, q2):
    """Update arm link positions"""
    L1 = 0.2
    L2 = 0.2
    
    # Link 1
    link1_x = 0.5 + (L1/2) * np.cos(q1)
...
# ============================================================================
print("\n" + "="*70)
print(" DEMO RUNNING - WATCH THE STEERING WHEEL!")
print("="*70)
print("\nLeft side: Virtual steering wheel (rotating)")
print("Right side: Piper arm (tracking wheel)")
...
try:
    ...
    while simulation_app.is_running():
        ...
        steer = np.sin(elapsed * 0.5) * 0.8  # -0.8 to +0.8
        
        # Convert to steering wheel angle (degrees)
        wheel_angle = steer * 450  # ±360 degrees rotation
        ...
        # Status update
        if step_count % 60 == 0:
            print(f"Wheel: {wheel_angle:+6.1f}° | Steer: {steer:+.2f} | Arm: ({np.degrees(q1):+6.1f}°, {np.degrees(q2):+6.1f}°)")
